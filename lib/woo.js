...
const availability = function(product) {
  switch (product.id) {
    // Anet 3D Drucker
    case 2573:
      return 'Beim Lieferant an Lager - ca. 7 Tage';
  }
  if (product.stock_status === 'onbackorder') {
    return 'Beim Lieferant an Lager - ca. 14 Tage';
  }
  if (product.stock_quantity < 0) {
    return 'Beim Lieferant an Lager - ca. 14 Tage';
  }
  ...
  products_serialized(mysql, function(products) {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      toppreise_products.push({
        ...
        image_link: products[i].images.length && products[i].images[0].src,
        price: products[i].price,
        categories: '',
        brand: getBrand(products[i]),
        shipping: shippingCost(products[i]),
        availability: products[i].stock_status,
        lieferfrist: deliveryTime(products[i]),
        kurzbeschrieb: products[i].short_description.replace(/<\/?[^>]+(>|$)/g, ""),
        Herstellernummer: getMpn(products[i]),
      });
    }
