...
var addOrderId = function(id, type, oid, connection, next) {
  utils.log('adding order id ' + id + ' of type ' + type);
...
var loadWarehouseAll = function(connection, callback) {
  ...
  connection.query(q, function(err, row) {
    if (err) {
      ...
      for (let i in row) {
        all[row[i].id] = row[i];
        utils.log('row:');utils.log(row[i]);
        all[row[i].id].orderids == '' ? [] : JSON.parse(all[row[i].id].orderids);
      }
      callback(null, all);
    }
  });
