...
var loadWarehouseAll = function(connection, callback) {
  ...
  var q = 'SELECT * FROM warehouse LEFT JOIN links ON (warehouse.id = links.pid)';
  connection.query(q, function(err, row) {
    if (err) {
      utils.log(err, 'mysql');
      callback(err);
...
var edit = function(id, key, value, connection, cb) {
  ...
  if (special[0] === 'warehouse') {
    var q = 'INSERT INTO warehouse (id, ' + key.split('_').splice(1).join('_') + ') VALUES(' + id + ', "' + value + '") ON DUPLICATE KEY UPDATE ' + key.split('_').splice(1).join('_') + '="' + value + '"';
    utils.log(q, 'mysql');
    connection.query(q, function(err, row) {
      if (err) {
        utils.log(err, 'mysql');
...
var editVariation = function(id, id2, key, value, connection, cb) {
  ...
  // Warehouse Properties
  if (special[0] === 'warehouse') {
    var q = 'INSERT INTO warehouse (id, ' + key.split('_').splice(1).join('_') + ') VALUES(' + id2 + ', "' + value + '") ON DUPLICATE KEY UPDATE ' + key.split('_').splice(1).join('_') + '="' + value + '"';
    utils.log(q, 'mysql');
    connection.query(q, function(err, row) {
      if (err) {
        utils.log(err, 'mysql');
 ...
 * 
 **/
var addProduct = function(title, snapshotUrl, connection, cb) {
  utils.log('-');
  utils.log('adding new product item');
  utils.log('Title: ' + title);
  utils.log('URL: ' + snapshotUrl);
  var data = {
    name: title,
    type: 'simple',
    manage_stock: true,
  ...
  WooCommerce.post('products', data, function(err, data, res) {
    ...
    let q = 'INSERT INTO warehouse (id) VALUES (' + id + ')';
    utils.log(q, 'mysql');
    connection.query(q, function(err, row) {
      if (err) {
        utils.log(err, 'mysql');
        ...
        utils.log(row);
        q = 'INSERT INTO links (link, type, timestamp, title, pid) VALUES ("' + snapshotUrl + '", "snapshot", ' + (Math.round(+new Date() / 1000)) + ', "Aliexpress SnapShot Link", ' + id + ')';
        utils.log(q, 'mysql');
        connection.query(q, function(err, row) {
          if (err) {
            utils.log(err, 'mysql');
