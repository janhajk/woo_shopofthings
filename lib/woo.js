...
const comparis_category = function(productCategory) {
...
const comparis = function(mysql, cb) {
  products_new(mysql, function(products) {
    let comparis_products = [];
    for (let i = 0; i < products.length; i++) {
      if (products[i].status !== 'publish') continue;
      let new_category = comparis_category(products[i].categories);
    ...
    cb(null, utils.json2xml(comparis_products));
  });
};
