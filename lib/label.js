...
var brother_label = function(urlQrcode, sku) {
    ...
    // Image: Logo
    xml += brother_image(logo, margin.left, margin.top, paperPrintable.width, 20);
    // Text: Product Page
    xml += brother_text('Product Page', { x: margin.left, y: paper.height - margin.bottom - 10, w: paperPrintable.width, h: 10, fontSize: 10.5, align: 'CENTER' });
    // Barcode
    xml += brother_qr(urlQrcode, 32.3, 26.4);
    // SKU
    xml += brother_text(sku, { x: paper.width - margin.right - 20, y: margin.top + 20, w: 20, h: paperPrintable.height - 20, fontSize: 9, angle: 270, valign: 'BOTTOM' });
    // Closeup
    xml += brother_end();
    return xml;
};
...
var brother_qr = function(link, x, y) {
    var xml = '';
    xml += '<barcode:barcode>';
    xml += '  <pt:objectStyle x="' + x + 'pt" y="' + y + 'pt" width="15mm" height="15mm" backColor="#FFFFFF" backPrintColorNumber="0" ropMode="COPYPEN" angle="0" anchor="TOPLEFT" flip="NONE">';
