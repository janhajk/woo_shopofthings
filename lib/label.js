...
var product = function(id, res, cb) {
    const woo = require('./woo.js');
    woo.productFromId(id, function(product) {
        utils.log(product);
        shorturl(product[0].permalink, function(error, bitlyLink) {
            // label.xml
            var label = brother_XmlProductLabel(bitlyLink.url, product[0].sku);
            makeFile(label, res, ['logo.bmp']);
            cb();
        });
    });
};
...
var brother_XmlProductLabel = function(urlQrcode, sku) {
    ...
    var xml = '';
    // XML Head    
    xml += brother_head(paper, margin, 'portrait');
 ...
 * 
 **/
var brother_text = function(text, params, paper) {
    if (paper == undefined) { paper = paper = { orientation: 'portrait' }; }
    if (params.x == undefined) { params.x = 0; }
    if (params.y == undefined) { params.y = 0; }
    if (params.w == undefined) { params.w = 0; }
    if (params.h == undefined) { params.h = 0; }
    ...
    if (text.constructor !== Array) {
        ...
        xml += '  <pt:data>' + text.join("\r") + '</pt:data>';
        for (let i in text) {
